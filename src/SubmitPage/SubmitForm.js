...
class SubmitForm extends React.Component {
		...
		if (this.props.location.state) {
			...
			this.setState({
				data: {
					...this.state.data,
					...selectedStoreData,
					[OPENING_TIME]: this.parseTimeAndRoundToNearestHalfHour(selectedStoreData[OPENING_TIME]),
					[CLOSING_TIME]: this.parseTimeAndRoundToNearestHalfHour(selectedStoreData[CLOSING_TIME])
				},
	...
	canBeSubmitted() {
		...
		return (
			data[SAFETY_OBSERVATIONS].length || data[USEFUL_INFORMATION].length || data[OPENING_TIME] || data[CLOSING_TIME]
		);
	}
	...
	isClosingTimeInvalid = () => {
		...
		if (formData[OPENING_TIME]) {
			isClosingTimeInvalid = isBefore(formData[CLOSING_TIME], formData[OPENING_TIME]);
		}
	...
	render() {
		...
		return (
			<React.Fragment>
				...
				>
					<Alert show key="form-submit-error" variant="danger" onClose={this.toggleErrorNotification} dismissible>
				...
				<Form onSubmit={(e) => this.onSubmit(e)}>
					<div className="container p-3">
						<h6 className="text-uppercase font-weight-bold mb-3">{translations.add_update_store}</h6>
									...
									<Col>
										<Form.Group controlId="formBasicOpenTimings">
											...
											<TimePicker
												...
												onChange={(time) => this.handleTimeChange(time, OPENING_TIME)}
											/>
										</Form.Group>
									</Col>
									<Col>
										<Form.Group controlId="formBasicCloseTimings">
											...
											<TimePicker
												...
												onChange={(time) => this.handleTimeChange(time, CLOSING_TIME)}
											/>
											{isClosingTimeInvalid && <p className="closing-time-error">{translations.closing_time_error}</p>}
										</Form.Group>
									</Col>
								</Row>
