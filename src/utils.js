...
export function findNearbyStores(currentLocation) {
...
export function transformGeocodeResult(results, location) {
  if (results && results.length) {
    ...
    return {
      ...
      locality: getAddressComponent(result.address_components, "neighborhood"),
...
export const isStoreType = (types) => {
  ...
  return (
    types && !types.filter((type) => invalidTypes.indexOf(type) >= 0).length
  );
};
